...
function Skeleton({ h = 200 }: { h?: number }) {
  return (
    <div style={{ height: h, background: "rgba(255,255,255,0.04)", borderRadius: 8 }} />
  );
}
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#0f172a", color: "#f1f5f9" }}>
        <div style={{ padding: "12px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", fontSize: 13, color: "#475569" }}>
          Cargando gemelo…
        </div>
        <div style={{ padding: "20px 24px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
          <div style={{ ...section, gridColumn: "span 3" }}><Skeleton h={100} /></div>
          <div style={{ ...section, gridColumn: "span 3" }}><Skeleton h={280} /></div>
          <div style={{ ...section, gridColumn: "span 2", height: 480, padding: 0, overflow: "hidden" }}><Skeleton h={480} /></div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={section}><Skeleton h={120} /></div>
            <div style={section}><Skeleton h={160} /></div>
          </div>
          <div style={section}><Skeleton h={200} /></div>
          <div style={section}><Skeleton h={200} /></div>
          <div style={section}><Skeleton h={200} /></div>
          <div style={{ ...section, gridColumn: "span 3" }}><Skeleton h={200} /></div>
        </div>
      </div>
    );
  }
  ...
  if (error || !user) {
    return (
      <div style={{ minHeight: "100vh", background: "#0f172a", color: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ fontSize: 16, color: "#ef4444" }}>{error ?? "Gemelo no encontrado"}</div>
        <Link href="/" style={{ color: "#475569", textDecoration: "none", fontSize: 13 }}>← Volver al universo</Link>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "#f1f5f9" }}>
      ...
      >
        ...
        {siniestroResult && (
          <div style={{ background: `${siniestroResult.color}15`, border: `1px solid ${siniestroResult.color}50`, borderRadius: 8, padding: "4px 10px", fontSize: 11, color: siniestroResult.color }}>
            🚨 Siniestro: {siniestroResult.category} ({siniestroResult.probability}%)
          </div>
        )}

        {asistenciaResult && (
          <div style={{ background: `${asistenciaResult.color}15`, border: `1px solid ${asistenciaResult.color}50`, borderRadius: 8, padding: "4px 10px", fontSize: 11, color: asistenciaResult.color }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ width: 44, height: 44, borderRadius: "50%", border: `2px solid ${color}`, display: "flex", alignItems: "center", justifyContent: "center", background: "#1e293b", flexShrink: 0 }}>
      ...
      <div style={{ padding: "20px 24px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        ...
        {/* Row 2: Indicadores de riesgo */}
        {siniestroResult && asistenciaResult && (
          <div style={{ gridColumn: "span 3", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={{ ...section, border: `1px solid ${siniestroResult.color}30` }}>
              <div style={{ ...sectionTitle, color: siniestroResult.color }}>🚨 Probabilidad de Siniestro</div>
              <ProbabilidadSiniestro result={siniestroResult} />
            </div>
            <div style={{ ...section, border: `1px solid ${asistenciaResult.color}30` }}>
              <div style={{ ...sectionTitle, color: asistenciaResult.color }}>🔧 Probabilidad de Asistencia</div>
              <ProbabilidadAsistencia result={asistenciaResult} asistencias={asistencias} />
            </div>
          </div>
        )}

        {/* Row 3: Mapa + columna derecha */}
        <div style={{ ...section, gridColumn: "span 2", padding: 0, overflow: "hidden", height: 480 }}>
          <MapaRutasClient
            viajes={viajes}
            eventos={eventos}
            center={center}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
          ...
          <div style={{ ...section, flex: 1 }}>
            ...
            {exposure && <RiesgoGeografico exposure={exposure} />}
          </div>
        </div>
        ...
        <div style={section}>
          <div style={sectionTitle}>
            Últimos viajes
            <span style={{ fontSize: 10, color: "#334155", fontWeight: 400, marginLeft: 6 }}>({user.total_viajes} total)</span>
          </div>
        ...
        {/* Row 5: Chat AI */}
        <div style={{ ...section, gridColumn: "span 3" }}>
          <div style={sectionTitle}>Chat con IA · Gemelo de {user.nombre.split(" ")[0]}</div>
          <ChatAI user={user} />
        </div>
      </div>
    </div>
